    ...
    for i, (image, label, mask) in enumerate(dataloader):
        ...
        with grad_req:
            ...
            if class_specific:
                ...
                prototypes_of_correct_class = torch.t(model.module.prototype_class_identity[:,label]).cpu()
                inverted_distances, _ = torch.max((max_dist - min_distances) * prototypes_of_correct_class, dim=1)
                cluster_cost = torch.mean(max_dist - inverted_distances)
                
                masked_prototypes_of_correct_class = torch.t(model.module.prototype_class_identity[:,label2]).cpu()
                ...
                if use_l1_mask:
                    l1_mask = 1 - torch.t(model.module.prototype_class_identity).cpu()
                ...
                all_white_mask = torch.ones(mask.shape[2], mask.shape[3]).cpu()
                bool_mask = input2==1
